...
/**
 * Extends ChessPiece; Includes full implementation for Queen piece
...
public class Queen extends ChessPiece {
	...
	public boolean checkHelper(int[] start, int[] end, ChessBoard board) {
		...
		if(start[0] != end[0] && start[1] != end[1] && Math.abs(start[0]-end[0]) != Math.abs(start[1]-end[1])){
			return false;
		}
		if(start[0] == end[0]){
			if(start[1]<end[1])
				for(int i = start[1]+1; i<end[1]; i++){
					if(board.getPiece(start[0],i)!= null){
						return false;
					}
				}
			else{
			...
			if(start[0]<end[0])
				for(int i = start[0]+1; i<end[0]; i++){
					if(board.getPiece(i,start[1])!= null){
						return false;
					}
				}
			else{
