...
/**
 * Extends ChessPiece; Includes full implementation for Knight piece
...
public class Knight extends ChessPiece{
	...
	public boolean checkHelper(int[] start, int[] end, ChessBoard board) {
		...
		if((Math.abs(start[0]-end[0])==2 && Math.abs(start[1]-end[1])==1)||(Math.abs(start[0]-end[0])==1 && Math.abs(start[1]-end[1])==2)){
			return true;
		}else{
			return false;
		}
